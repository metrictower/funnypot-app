...
- **E6 (T6) — Escape everything reflected.** Search, PA broadcast, signage message, room title, add-note, receipt `arg` → `esc()`, slugify where it is a path/href. **PIN/access codes are never reflected at all.** *Fake-persistence exception (FP-0158):* a note/message/edit POSTed to a write endpoint (HR profile edit, signage broadcast) IS now echoed back on a later read of the same view — but only through the same `esc()` (never executable, no stored XSS), keyed per ip + persona seed, bounded (per-value length + per-view count + global row cap) and TTL'd so it evaporates, and never for a PIN/access-code field. `FakePersistenceStore` + `FakePersistence` (fail-open; a persistable view bypasses the byte-identical panel cache so one visitor's echo is never frozen or served to another).
