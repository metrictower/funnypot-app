...
| **Pure-PHP SSH-2.0 server** | Real OpenSSH-8.9p1-shaped SSH-2.0 transport: curve25519 / ECDH P-256/384/521 / DH group14/16/18 + group-exchange key exchange, RSA-3072 (rsa-sha2-256/512), ECDSA P-256 and ed25519 host keys, chacha20-poly1305 / AES-CTR / AES-GCM ciphers with HMAC-SHA1/SHA2 (E&M + ETM) and UMAC, delayed zlib compression, strict-kex (Terrapin) and EXT_INFO — hasshServer 779664e6 (a real OpenSSH 8.2/8.4 fingerprint; exact 8.9p1 parity follows with sntrup761). No libssh, no OpenSSH. Accept-all auth drops the attacker into a fake shell with decoy files. |
| **TCP protocol emulators** (21) | ssh, telnet, redis, ftp, smtp, memcached, pop3, imap, finger, vnc, rsync, clamav, zookeeper, mysql, postgres, mongodb, modbus, ethernet-ip, **rdp**, **smb**, **tr069**. Every command logged, nothing run. |
