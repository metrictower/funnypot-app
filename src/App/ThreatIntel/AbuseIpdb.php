...
final class AbuseIpdb
{
    ...
    public function drain(int $limit = 200): array
    {
        ...
        foreach ($rows as $row) {
            ...
            // FP-0247 (opus #1): guard every terminal mutation on our own claim token. If this pass was
            // slow enough (>10 min) that the 10-min stale reclaim handed the row to a newer overlapping
            // pass, our delete/update must NOT touch it — the newer owner is now authoritative, so a
            // stale finisher can neither drop a row the new pass is retrying nor delete its work.
            if ($status >= 200 && $status < 300) {
                ...
                if ($attempts >= 3) {
                    ...
                    $this->db()->prepare('UPDATE abuse_queue SET attempts = :a WHERE id = :id AND claim = :tok')
    ...
    /**
     * FP-0247 (opus #1): delete a row only while we still hold its claim. A slow pass whose rows were
     * stale-reclaimed by a newer overlapping pass must not delete the newer owner's work.
     */
    private function deleteClaimed(int $id, string $token): void
    {
        $this->db()->prepare('DELETE FROM abuse_queue WHERE id = :id AND claim = :tok')
