...
/**
 * The five RFC 3526 MODP groups (2048/3072/4096/6144/8192, generator 2) shared by {@see Dh} and
 * {@see DhGex}, plus the big-endian integer helpers those two need. The moduli are embedded as hex
 * constants (PHP has no API for OpenSSL's named DH groups, so embedding is the only option) and
 * decoded once; each SHA-256 pins to the RFC value in {@see \Funnypot\Tests\SshKexTest}.
 *
 * The helpers are pure PHP and 8.0-safe (no gmp/bcmath): cmp/bitsSet/minusOne over raw big-endian
 * bytes are all that the DH peer-value validation and RFC 4419 group choice require.
 */
final class DhGroups
{
    public const G = "\x02";

    // The alternate generator for {@see DhComputation::dhDerive()}'s subgroup-check retry. Building both
    // DH keys with a non-standard generator makes OpenSSL 3 treat the modulus as an anonymous group (no
    // named-group q attached), so it skips the small-subgroup test and derives an in-range non-subgroup e
    // exactly as sshd's dh_pub_is_valid does. g plays no part in e^x mod p, so K is identical to the g=2
    // result; range/popcount validation ({@see DhComputation::validatePeerValue()}) stays load-bearing.
    public const G_RETRY = "\x05";
    ...
    private const HEX = [
        ...
        6144 => '',
    ...
    /** @var array<int,string> raw big-endian moduli, memoised on first use */
    private static array $cache = [];

    /** The five embedded sizes in ascending order (bits). @return int[] */
    public static function bits(): array
    {
        return [2048, 3072, 4096, 6144, 8192];
    }

    /** The raw big-endian modulus for a supported bit size; throws on any other size. */
    public static function modulus(int $bits): string
    {
        if (!isset(self::HEX[$bits])) {
            throw new \RuntimeException("ssh: no embedded DH group of {$bits} bits");
        }

        return self::$cache[$bits] ??= (string) hex2bin(self::HEX[$bits]);
    }

    /**
     * RFC 4419 group choice (OpenSSH choose_dh): among the embedded sizes within [$min, $max], the
     * smallest that is >= $n; if none is >= $n, the largest in range; if the range holds no embedded
     * size at all, throw (⇒ disconnect, matching sshd's "no matching DH grp" path). Inputs are the
     * clamped min/max the caller derived.
     */
    public static function choose(int $min, int $n, int $max): int
    {
        $candidates = [];
        foreach (self::bits() as $size) {
            if ($size >= $min && $size <= $max) {
                $candidates[] = $size;
            }
        }
        if ($candidates === []) {
            throw new \RuntimeException('ssh: no matching DH group');
        }
        foreach ($candidates as $size) {
            if ($size >= $n) {
                return $size; // ascending, so the first >= n is the smallest
            }
        }

        return $candidates[count($candidates) - 1]; // largest in range, all below n
    }

    /** Compare two unsigned big-endian magnitudes: -1 if a<b, 0 if equal, 1 if a>b. */
    public static function cmp(string $a, string $b): int
    {
        ...
        return $a === $b ? 0 : (strcmp($a, $b) < 0 ? -1 : 1);
    }

    /** Population count (number of set bits) of an unsigned big-endian magnitude. */
    public static function bitsSet(string $m): int
    {
        $count = 0;
        $len = strlen($m);
        for ($i = 0; $i < $len; $i++) {
            $count += substr_count(decbin(ord($m[$i])), '1');
        }

        return $count;
    }

    /** p − 1 for an odd modulus p: clearing the low bit suffices (no borrow). */
    public static function minusOne(string $p): string
    {
        $last = strlen($p) - 1;
        $p[$last] = chr(ord($p[$last]) & 0xfe);
